    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-100 via-gray-50 to-blue-50">
      ...
      <header className="relative overflow-hidden bg-gradient-to-r from-slate-800 via-blue-900 to-slate-700 text-white shadow-2xl">
        ...
        <div className="relative px-2 sm:px-4 lg:px-8 w-full">
          <div className="flex flex-row items-center h-16 w-full relative">
            ...
            <div className="flex items-center flex-shrink-0 z-10">
              <button
                ...
                className="md:hidden p-2 rounded-xl hover:bg-white/20 backdrop-blur-sm transition-all duration-200"
              >
            ...
            <div className="flex-1 flex justify-center items-center absolute left-0 right-0 pointer-events-none">
              <h1 className="text-base sm:text-xl font-bold truncate max-w-[70vw] sm:max-w-none pointer-events-auto bg-gradient-to-r from-white to-blue-100 bg-clip-text text-transparent">
          ...
          <div className="flex flex-col sm:flex-row items-center justify-center sm:justify-end space-y-2 sm:space-y-0 sm:space-x-4 mt-2 sm:mt-0 w-full">
            ...
            <div className="flex items-center space-x-1 sm:space-x-2 bg-white/10 px-3 py-1.5 rounded-lg backdrop-blur-sm">
                {isConnected ? (
              ...
              <div className="relative">
                <button
                  ...
                  className="relative p-2 rounded-xl hover:bg-white/20 backdrop-blur-sm transition-all duration-200"
                >
                  <Bell size={20} />
                ...
                <div className="z-50">
                  ...
                  <div className="block sm:hidden fixed top-20 left-1/2 transform -translate-x-1/2 w-11/12 max-w-xs bg-white/95 backdrop-blur-sm rounded-2xl shadow-2xl border border-white/20 z-50 p-4" onClick={e => e.stopPropagation()}>
                    ...
                    <div className="max-h-64 overflow-y-auto">
                      {notifications.length > 0 ? (
                        notifications.map((item, index) => (
                          <div key={index} className="p-3 border-b border-gray-200/50 last:border-b-0 hover:bg-gray-50/50 rounded-lg transition-colors">
                    ...
                    <button className="mt-4 w-full bg-gradient-to-r from-[#2E8B57] to-[#3B82F6] text-white rounded-xl py-2 font-semibold shadow-lg hover:shadow-xl transition-all duration-200" onClick={() => setShowNotifications(false)}>Close</button>
                  </div>
                  {/* Desktop: right-aligned dropdown */}
                  <div className="hidden sm:block absolute right-0 mt-2 w-80 bg-white/95 backdrop-blur-sm rounded-2xl shadow-2xl border border-white/20 z-50">
            ...
            <span className="text-xs sm:text-sm truncate max-w-[60vw] sm:max-w-none text-center bg-white/10 px-3 py-1.5 rounded-lg backdrop-blur-sm">Welcome, {user?.username}</span>
              <button
                onClick={handleLogout}
              className="flex items-center justify-center space-x-1 px-2 py-2 sm:px-3 rounded-xl hover:bg-white/20 backdrop-blur-sm transition-all duration-200"
              >
                <LogOut size={16} />
      ...
      <div className="flex">
        ...
        <aside 
          ...
          } fixed inset-y-0 left-0 z-30 w-4/5 max-w-xs bg-white/95 backdrop-blur-sm shadow-2xl border-r border-white/20 transform transition-transform duration-300 ease-in-out md:translate-x-0 md:static md:inset-0 mt-16 md:mt-0`}
        >
        ...
        <main className="flex-1 p-2 sm:p-4 md:p-6 md:ml-0">
          <div className="bg-white/80 backdrop-blur-sm rounded-2xl shadow-xl border border-white/20 min-h-[calc(100vh-8rem)]">
